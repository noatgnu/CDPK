...
class GOEnrichmentResult(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        studyfile = self.request.files['study'][0]
        populationfile = self.request.files['population'][0]
        associationfile = self.request.files['association'][0]
...
class UniparserForm(tornado.web.RequestHandler):
    def get(self):
        self.render("uniparser2.html", title="Uniprot Parser", dboptions=settingup.DBWEB)
